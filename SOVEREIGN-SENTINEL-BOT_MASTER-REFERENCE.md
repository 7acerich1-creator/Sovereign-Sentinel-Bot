...
1. **CE-4 FIX: Threads 500-char truncation.** Meta's Threads API hard-limits posts to 500 characters. Distribution now truncates Threads text to 497 chars + "..." automatically. `THREADS_CHAR_LIMIT` constant at line 46.
2. **CE-5 FIX: Instagram `type: post` param.** IG image posts require explicit `type: post` in Buffer GraphQL mutation (otherwise Buffer defaults to Reel, which fails for static images). Now injected conditionally when `service === "instagram" && draft.media_url` exists.
3. **YouTube RE-ENABLED for community posts.** Initially skipped YouTube (wrong assumption it needed video). Ace confirmed YouTube community posts support image+text via Buffer — reverted skip immediately. YouTube now flows through distribution like all other platforms.
4. **End-to-end verification complete.** All 6 platform paths traced through distribution sweep: X ✅, TikTok ✅, Threads ✅ (CE-4), Instagram ✅ (CE-5), YouTube ✅ (re-enabled), LinkedIn ✅. No platform omitted. Content generated by Deterministic Content Engine with brand voice (Sovereign Synthesis / Containment Field), niche rotation, and platform-native formatting.
5. **Push status: ✅ PUSHED** — Commit `e218e1d` pushed to main. Railway auto-deployed.

**NEXT SESSION PRIORITIES:**
1. **Monitor 6:30 AM ET cron** — verify autonomous production hits ALL platforms including IG, YouTube, Threads. Check Buffer calendar for posts across all 9 channels.
2. **VidRush pipeline end-to-end verification** — Make.com Scenarios E (YouTube transcription) + F (Clip Pipeline). Get webhook URLs, set `MAKE_SCENARIO_E_WEBHOOK` and `MAKE_SCENARIO_F_WEBHOOK` in Railway.
3. **Clean Buffer queue** — verify only quality brand-voiced content is scheduled. Remove any stale/test posts.
4. **Image provider fallback** (deferred) — Imagen 4 is sole provider. Consider Together AI FLUX.1 if quota becomes issue.
...
- **Instagram:** Requires `type: post` for image posts. Without it, Buffer defaults to Reel which fails for static images. Code: CE-5 fix, line ~709 of content-engine.ts.
- **YouTube:** Supports community posts (image + text) via Buffer. NO special params needed — standard mutation works. Initially skipped in error, reverted same session.
