...
| Field | Value |
|---|---|
| **Current phase** | Phase 2 Orchestration — **Task 2.1 + 2.2 ☑, Task 2.3 + 2.4 ☐**. Railway ↔ Pod contract is now code-complete on the client side: typed orchestration surface + TS/Python shared-schema models landed this session. No pod has been created yet from `runpod-client.ts` — contract test (Task 2.3) + wake/sleep wrapper (Task 2.4) are the remaining phase gates. |
| **Current phase status** | **Phase 0: ALL ☑. Phase 1: ALL ☑. Phase 2: 2/4 ☑ (2.1 + 2.2 done).** `src/pod/runpod-client.ts` ships `startPod/waitUntilReady/produceVideo/stopPod/fetchHealth` with create-per-job semantics (no hard-coded `RUNPOD_POD_ID`), H100→A100 GPU fallback (D1), env forwarding for `POD_WORKER_TOKEN` + XTTS speakers + R2 + Supabase, exponential-backoff retry on 429/5xx, typed errors (`PodContractError` / `RunPodApiError` / `PodJobFailedError`), and a client-side `validateJobSpec()` mirror of the Pydantic scene-index contiguity rule. `src/pod/types.ts` + `pod/models.py` are bit-for-bit length-bound mirrors. `npx tsc --noEmit` clean. `ast.parse` on models.py OK. Image at `ghcr.io/7acerich1-creator/sovereign-sentinel-pod:latest` (digest `sha256:00212d098b3f…`). Network volume `gai851lcfw` still carries the S62-uploaded speaker WAVs. Active pod count: 0. |
| **Total phases** | 8 (Phase 0 → Phase 7) |
| **Last session** | Session 63 — 2026-04-15 — Shipped Phase 2 Tasks 2.1 + 2.2. Wrote `src/pod/types.ts` (Brand / Scene / JobSpec / ProduceAccepted / JobResult / ArtifactUrls / HealthReport / PodHandle + 3 typed error classes), `src/pod/runpod-client.ts` (RunPod REST v1 orchestration client, 530 lines, worker URL pattern `https://<podId>-8000.proxy.runpod.net`, retry/backoff, env forwarding allowlist), and `pod/models.py` (Pydantic mirror as the canonical Python source — `pod/worker.py` still duplicates inline but Phase 4 rewrite will collapse). Ran `npx tsc --noEmit` → clean. `ast.parse pod/models.py` → OK. Did NOT touch `pod/worker.py` (avoids triggering a GHA Docker rebuild for a pure skeleton/typing change). Respected no-hardcoded-pod-id directive from S62. |
| **Last commit touching this work** | *(pending push — S63 local working tree: 3 new files under src/pod/ + pod/models.py + this STATUS block)* |
| **Blocker** | NONE. Task 2.3 (contract test) is next — requires a live pod to hit, so Task 2.4 (withPodSession wrapper) is the natural co-task (same session creates pod, runs contract, tears it down). |
| **Next session's first action** | **Phase 2 Task 2.4 FIRST (enables 2.3):** create `src/pod/session.ts` with `withPodSession(async (handle) => { ... }, { idleWindowMs: 600_000 })` — wakes on entry via `startPod()` + `waitUntilReady()`, sleeps on exit via `stopPod()` after the D6 10-min idle window elapses (hold-open cache is session-scoped; fine as in-memory map for the single-process orchestrator). Then Task 2.3: add `scripts/test-pod-contract.ts` + `npm run test:pod-contract` that uses `withPodSession` to spin up a real pod, POSTs a minimal 1-scene JobSpec (stub worker currently transitions `queued → running → done` in ~1s with null artifact URLs — assert the round-trip, expect `PodJobFailedError` on the artifact-null guard, catch-and-ignore that specific error path until Phase 4 wires real artifacts). Budget: ≤$0.50 per test run. Verify `tsc --noEmit` + `npm run test:pod-contract` both green before closing. |
