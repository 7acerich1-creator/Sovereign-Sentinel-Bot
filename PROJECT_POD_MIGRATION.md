...
| Field | Value |
|---|---|
| **Current phase** | **PHASE 4 IN PROGRESS — 2026-04-16 S67.** Pod-side pipeline modules written: `pod/pipelines/xtts.py` (XTTSv2 inference), `pod/pipelines/flux.py` (FLUX.1 [dev] bf16), `pod/pipelines/compose.py` (Ken Burns + ffmpeg), `pod/pipelines/r2.py` (Cloudflare R2 upload). `pod/worker.py` upgraded from Phase 1 skeleton to real pipeline orchestration (v1.0.0). Phase 3 commits confirmed on `origin/main` at `27ef5ab`. **Task 4.1 is CODE-COMPLETE locally (not yet committed).** Tasks 4.2-4.5 (Railway-side refactoring of faceless-factory.ts to delegate to pod) remain. |
| **Current phase status** | **Phase 0: ALL done. Phase 1: ALL done. Phase 2: 4/4 done. Phase 3: 7/7 done (PUSHED to origin/main at 27ef5ab). Phase 4: Task 4.1 CODE-COMPLETE (local), Tasks 4.2-4.5 pending.** S67 built: (4.1a) `pod/pipelines/xtts.py` -- lazy model singleton, speaker latent caching, per-scene WAV synthesis + ffmpeg concat. (4.1b) `pod/pipelines/flux.py` -- FLUX.1 [dev] bf16 pipeline from diffusers, 1024x1024 @ 30 steps / 3.5 guidance per D2. (4.1c) `pod/pipelines/compose.py` -- Ken Burns zoompan with alternating zoom-in/out, per-scene clip assembly, ffmpeg concat to final MP4, thumbnail extraction. (4.1d) `pod/pipelines/r2.py` -- boto3 S3 client with R2 endpoint override, multipart upload for large videos, public URL construction. (4.1e) `pod/worker.py` v1.0.0 -- real `_run_pipeline()` replacing `_run_job_stub()`, 4-stage orchestration (XTTS -> FLUX -> compose -> R2), job dir cleanup, error propagation to `/jobs/{id}`. All Python `ast.parse` clean. `npx tsc --noEmit` clean. Dockerfile + requirements.txt already cover all deps (no changes needed). |
| **Total phases** | 8 (Phase 0 -> Phase 7) |
| **Last session** | Session 67 -- 2026-04-16 -- **PHASE 4 Task 4.1 CODE-COMPLETE.** Wrote 4 pipeline modules (xtts.py 249L, flux.py 155L, compose.py 273L, r2.py 146L) + rewrote worker.py (378L) from skeleton to real orchestration. Phase 3 confirmed pushed and live on Railway (27ef5ab). No truncation issues (used bash+python for all writes >300 lines per hard rule). |
| **Last commit touching this work** | `27ef5ab` (origin/main) -- Phase 3 widen-pass. Phase 4 work is LOCAL ONLY, not yet committed. |
| **Blocker** | **R2 credentials.** `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET_VIDEOS` must be set on Railway (forwarded to pod env). Without these, the pod pipeline falls back to `file://` URLs which Railway cannot download. Ace needs to create an R2 bucket + API token in Cloudflare dashboard. |
| **Next session's first action** | **If R2 creds are set:** commit Phase 4.1 pod code, push, rebuild Docker image (GH Actions pod-build.yml), then start Task 4.2 (refactor faceless-factory.ts to delegate to pod). **If R2 creds NOT set:** surface the Cloudflare R2 setup steps to Ace as a 5-minute browser task, then proceed with Task 4.2 Railway-side refactoring in parallel. 
