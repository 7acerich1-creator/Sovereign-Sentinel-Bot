...
| Field | Value |
|---|---|
| **Current phase** | **PHASE 3 CODE-COMPLETE ✅ 2026-04-15 S66.** All seven Phase 3 tasks shipped locally on commit `0ec0325` (NOT pushed — Railway auto-deploys and would kill any live pipeline; push is a manual step for next session after confirming the fleet is idle). Brand correctness is now enforced at the INTAKE layer: Alfred emits two brand-bound seeds per day (`PIPELINE_IDEA_ACE_RICHIE` + `PIPELINE_IDEA_CONTAINMENT_FIELD`), faceless-factory hard-fails on allowlist violations, niches are on 30d hard / 14d soft-relax cooldown per brand, and scripts are embedded + Pinecone-deduped before render with a 2-retry budget. **Next active phase: Phase 4 — Railway /produce real logic (R2 storage + pod-side FLUX/XTTS + download-back-to-Railway for distribution).** |
| **Current phase status** | **Phase 0: ALL ☑. Phase 1: ALL ☑. Phase 2: 4/4 ☑. Phase 3: 7/7 ☑ (CODE-COMPLETE, push-pending).** S66 shipped: (3.1) S48 re-verified on origin/main — no-op confirmed. (3.2) `BRAND_NICHE_ALLOWLIST` in `src/data/shared-context.ts` + `isAllowedNiche()` + `normalizeNiche()` + `getAllowedNiches()`. (3.3) Alfred directive now emits dual `PIPELINE_IDEA_<BRAND>: niche :: thesis` contracts; regex parser populates `{ace_richie_seed, tcf_seed}` tuple. (3.4) `BrandNicheViolation` thrown at entry of `produceFacelessVideo` — no model call, no disk write, no R2 before the guard. (3.5) `supabase/migrations/004_niche_cooldown.sql` table+view+RLS APPLIED to live Supabase `wzthxohtgojenukmdubz`; `src/tools/niche-cooldown.ts` with `getNicheCooldownSnapshot` / `cooldownSummaryLine` / `assertNichePermitted` / `recordNicheRun`. Alfred directive built at runtime (`buildAlfredDailyScanDirective` async) — injects live cooldown per brand at dispatch. `recordNicheRun` fires post-`executeFullPipeline` in the auto-pipeline bridge. (3.6) `src/tools/script-uniqueness-guard.ts` — Gemini `gemini-embedding-001` 1024d primary / OpenAI `text-embedding-3-small` 768d fallback; Pinecone REST query against `scripts-<brand>` namespace topK=5; `UNIQUENESS_THRESHOLD=0.85` cosine; `ScriptTooSimilarError` + `assertScriptUnique` + non-throwing `checkScriptUniqueness`. Writer wrapped in 2-retry loop inside faceless-factory before halt. (3.7) `persistShippedScript` upserts vector `script-<brand>-<jobId>` into brand namespace post-upload — fire-and-forget, never blocks completion. Graceful degradation everywhere (missing Pinecone / Supabase / embedding keys → warn + permit, never halt). `npx tsc --noEmit` clean. Active pod count: 0. |
| **Total phases** | 8 (Phase 0 → Phase 7) |
| **Last session** | Session 66 — 2026-04-15 — **PHASE 3 CODE-COMPLETE.** Built niche-cooldown.ts + script-uniqueness-guard.ts + migration 004_niche_cooldown.sql; wired cooldown summary into Alfred's async directive builder; wrapped script writer in uniqueness retry loop; added persistShippedScript call post-upload; migration APPLIED to live Supabase. Recovered faceless-factory.ts from two Edit-tool truncations per `feedback_edit_truncation_recovery.md` (git-show + python stitch). Committed as `0ec0325` locally. **NOT pushed** per `feedback_no_push_during_pipeline.md` — next session confirms fleet idle, pushes, polls `gh run list` to CI-green per `feedback_verify_ci_before_closing_task.md`. |
| **Last commit touching this work** | `0ec0325` (local only, main) — "feat(phase3): brand-niche intake + cooldown + script uniqueness guard (Tasks 3.3-3.7)" |
| **Blocker** | None. Phase 3 green locally. |
| **Next session's first action** | **Push `0ec0325` to origin/main** (confirm pipeline idle first — no active `produceFacelessVideo` runs on Railway), then poll `gh run list` until CI conclusion is non-empty (success). Then begin **Phase 4 Task 4.1** — R2 bucket credentials into Railway env + pod-side upload logic replacing the Supabase Storage fallback. |
