...
| **Last session** | Session 75 -- 2026-04-17 -- Task 5.12 partial + orphan pod safety net. (1) Docker image rebuilt (GH Actions green). Test script committed. 3 orphan pods terminated. Live test blocked by US-KS-2 SUPPLY_CONSTRAINT. (2) SIGTERM/SIGINT handlers wired into `index.ts` + both test scripts — calls `shutdownPodSession()` + `sweepStalePods()`. (3) New `listSovereignPods()` (GraphQL) + `sweepStalePods()` in `runpod-client.ts` — kills any sovereign-worker pod >30min on shutdown. (4) Task 7.5 added: batch production window mode. Commits `e0ed7db` + `90082d9` + `c2a8870`. |
| **Last commit touching this work** | `c2a8870` (origin/main) -- S75 orphan pod safety net + batch production task. |
