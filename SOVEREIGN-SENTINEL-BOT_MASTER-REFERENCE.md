...
**Last trimmed:** 2026-05-06 (S131 — Anita newsletter realignment. Four drift points fixed in one sweep: (1) `geminiComposeIssue` hardcoded the dark email wrapper instead of light canonical; rewritten to return structured `NewsletterContent` + dedicated `renderNewsletterEmail` (light canonical per `.claude/skills/brand-identity/resources/email-templates.md`) + `renderNewsletterWebPage` (matches funnel article-page CSS). (2) `listNewsletterRecipients` now unions `newsletter_subscribers` (active, unsubscribed_at IS NULL) + `initiates` (payment_status != 'unsubscribed'), deduped on lowercased email. (3) `bot_active_state.active_format` for anita pointed at the removed `email_html_dark_wrapper` key; updated to `email_html_light_canonical`. (4) Idea graph now has `kind` ('mechanism'|'foundation'|'open'), `mechanism_sequence`, `cta_path` columns. 9 mechanism rows seeded for issues 02-10 (approval_reflex → containment_field_synthesis); 7 foundations pinned (Simulation, Glitch, SovSynth, ContainmentField, BiologicalDrag, EscapeVelocity, LighthouseStance); Protocol 77 dropped from foundation set. Picker pulls mechanisms in `mechanism_sequence` order then falls through to open mode after exhaustion. New: `publishToWebsite()` writes each new issue as `/newsletter/{slug}/index.html` and prepends a row to `/newsletter/index.html` in the `sovereign-landing` repo via GitHub Contents API. Graceful no-op when `GITHUB_TOKEN` is unset (logs warning, send still succeeds). Required new env var: `GITHUB_TOKEN` on Railway with `repo` scope. Optional: `GITHUB_OWNER` (default `7acerich1-creator`), `GITHUB_REPO` (default `sovereign-landing`), `GITHUB_BRANCH` (default `main`). New `newsletter_issues` columns: `slug`, `structured_content` (jsonb), `published_to_website_at`, `web_archive_url`. Issue 01 backfilled. 4 test rows cleaned from `newsletter_subscribers`. `/newsletter/index.html` archive layout updated with a pinned Foundations grid above the chronological news-list — visual stubs only; foundation copy gets written in a focused later session.)

**Last trimmed:** 2026-05-03 (S129b — Full sweep of `_fat.ts` for the same param-remap bug pattern. Audit script (`/tmp/fat_audit.py`) flagged 8 fat tools with mismatches across 13 actions. **6 actions were completely broken** (calendar.create sent `title` but narrow read `summary`; family.save sent `dob` not `date_of_birth`; research.analyze_pdf sent `attachment_id` not `file_id`; self.create_piece sent `body` not `value`; plan.record_step sent `step_index` not `step_idx`; plan.record_artifact sent `artifact_id` not `artifact`+`target_name`). 2 actions had wrong-time-window params (calendar.list/days_ahead vs time_min/time_max). Other tools had capability gaps (gmail inbox/search missing `hours_back`/`max`/`unread_only`; memory.recall missing `key_match`/`max`; followups.list/learning.* missing limit/status/snippet/related_subjects/what_was_tried). All fixed via dispatch-layer remap accepting both old fat names and narrow names. Re-run audit shows zero remaining mismatches. Commits c568192 + S129b commit.)
...
- **Fat-tool dispatch must remap params** — `_fat.ts` tools wrap narrow `*Tool` classes that have their own arg names. When adding/editing a fat tool, verify the schema field names match what the narrow tool reads, OR remap inside `execute()` before calling `narrowT.execute(args)`. The reminders tool was silently broken this way for ~2 weeks (S129); a full audit at S129b found 8 fat tools with the same bug, 6 actions completely broken. Audit script: `/tmp/fat_audit.py` (regenerable from `_fat.ts` + narrow files). Pattern check: `grep "args\\." src/tools/sapphire/<narrow>.ts` and compare to the fat schema's `parameters` keys.
