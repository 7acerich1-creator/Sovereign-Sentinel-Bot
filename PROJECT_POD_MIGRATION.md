...
| Field | Value |
|---|---|
| **Current phase** | **PHASE 5 IN PROGRESS — 2026-04-16 S73.** Tasks 5.1-5.10, 5.13, 5.14 done (12/14). Kinetic captions shipped — GPU Whisper on pod. |
| **Current phase status** | **Phase 0-4: ALL done. Phase 5: 12/14 done (5.1 ☑, 5.2 ☑, 5.3 ☑, 5.4 ☑, 5.5 ☑, 5.6 ☑, 5.7 ☑, 5.8 ☑, 5.9 ☑, 5.10 ☑, 5.13 ☑, 5.14 ☑, 5.11-5.12 ☐).** |
| **Total phases** | 8 (Phase 0 -> Phase 7) |
| **Last session** | Session 73 -- 2026-04-16 -- Task 5.10 shipped: kinetic captions via GPU Whisper on pod. `faster-whisper` added to requirements. `compose.py` gains `generate_and_burn_captions()` pipeline: extract audio -> faster-whisper large-v3 word-level timestamps on CUDA -> chunk into 2-4 word bursts -> brand-specific ASS (TCF: Bebas Neue uppercase silver no-box, Ace: Montserrat SemiBold gold no-box, both with scale pop-in animation) -> ffmpeg subtitles burn. Captions skip opening sequence (5.0s). Green opaque-box captions retired. Commit `d446a8b`. |
| **Last commit touching this work** | `d446a8b` (origin/main) -- S73 Task 5.10 kinetic captions. |
| **Blocker** | **None.** |
| **Next session's first action** | **Task 5.11** — Composite audio mixing in compose.py. Then 5.12 (Docker rebuild + live pod test). Two tasks remain to close Phase 5. |
