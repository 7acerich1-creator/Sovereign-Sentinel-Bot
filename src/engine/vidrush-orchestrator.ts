...
// curateShorts replaced by generateStandaloneShorts in forward pipeline.
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// @deprecated — Phase 5 Task 5.5: RETIRED.
...
async function uploadClipsToStorage(clips: ClipMeta[], jobId: string, meta?: { brand?: string; niche?: string; title?: string }): Promise<void> {
  ...
  for (const clip of clips) {
    ...
    // FIX: Pod-rendered clips already have an R2 publicUrl from
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// — STEP 2.5: LONG-FORM YOUTUBE DESCRIPTION VIA VIRAL BRAIN
...
async function generateLongFormDescription(
  ...
  // Brand voice is the FLOOR, not the ceiling. The previous prompt
...
async function distributeClips(
  ...
  for (const clip of uploadedClips) {
    ...
    // 10s delay — Buffer allows 100 req/15min (shared limiter also enforces)
...
async function scheduleBufferWeek(
  ...
  // Pre-flight — don't attempt scheduling if quota is already blown
  ...
  for (let clipIdx = 0; clipIdx < clips.length; clipIdx++) {
    // Bail out mid-loop if quota gets blown during scheduling
    if (isBufferQuotaExhausted()) {
...
export interface PipelineOptions {
  ...
  // Scheduled YouTube publish time. ISO 8601. When set, video uploads
...
export async function executeFullPipeline(
  ...
  // Kill the hardcoded "Extracted intelligence from the simulation" payload.
  ...
  if (dryRun) {
    ...
    try {
      // FIX 3 OVERRIDE: description is now the Viral Brain output, not a
      ...
      // Batch scheduled publishing — stagger long-forms across the day
      ...
      // Auto-comment with diagnostic link on every successful upload.
  ...
  if (dryRun) {
    ...
    try {
      if (!llm) {
        ...
        // Single pod session for TTS (XTTS) + render — everything on GPU.
        try {
          await withPodSession(async (handle) => {
            ...
            for (let i = 0; i < standaloneShorts.length; i++) {
              ...
              const spec: ShortJobSpec = {
                ...
                // dual-field thumbnail (headline + subhead) + backward-compat alias.
