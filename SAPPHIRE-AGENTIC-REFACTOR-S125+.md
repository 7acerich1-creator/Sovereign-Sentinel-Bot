...
**Phase 8 (✅ shipped 2026-04-30, same session as 1-7):** Crew memory infrastructure generalized. Table migrated (`sapphire_core_memory` → `agent_core_memory` + agent_name column). Memory tools agent-aware via ToolContext. MemoryTool added to global tools array (Anita + crew can now call memory). Sleeptime consolidator iterates over all 6 agents. Anita's core memory bootstrapped with 5 marketing-relevant slots. Per-agent reflection schedulers deferred to Phase 9 (need per-agent diary tables first). See master ref entry.

**Phase 7 (✅ shipped 2026-04-30, same session as 1-6):** Anita elevated to Marketing Lead (Anthropic Sonnet 4 + marketing fat tool + 2 Supabase tables + doctrine overhaul). Veritas elevated to Chief of Staff (Anthropic, role placeholder pending more strategic thinking). Alfred expanded to Content Production Lead. No cross-crew dispatch for Anita yet (Architect stays in loop). See master ref entry for full details.
